...
namespace pandaform.API
{
    ...
    public class registration : System.Web.Services.WebService
    {
        ...
        [WebMethod]
        public void registrationsubmit(string jatiname, int vahinum, string vahiname, int pageno, int year, string name, string dob, string gender, string adharnum, string pannum, string pname, string sasurname, string childrenname, string mobilenum, string phonenum, string email, string pad, string work, string ghousenum, string gvillagename, string gtahasil, string gthana, string gjila, string gshahar, string gstate, string gdesh, string gpincode, string vhousenum, string vvillagename, string vtahasil, string vthana, string vjila, string vshahar, string vstate, string vdesh, string vpincode, string signature, string photo, string remark, string date)
        {
            SqlHelper.ExecuteNonQuery(CommandType.Text, "insert into registration(jaatiname,vahinum,vahiname,pageno,year,name,dob,gender,adharnum,pannum,pname,sasurname,childrenname,mobilenum,phonenum,email,pad,work,ghousenum,gvillagename,gtahasil,gthana,gjila,gshahar,gstate,gdesh,gpincode,vhousenum,vvillagename,vtahasil,vthana,vjila,vshahar,vstate,vdesh,vpincode,signature,photo,remark,date) values('" + pcase.ToTitleCase(jatiname) + "','" + vahinum + "','" + vahiname + "','" + pageno + "','" + year + "','" + name + "','" + dob + "','" + gender + "','" + adharnum + "','" + pannum + "','" + pname + "','" + sasurname + "','" + childrenname + "','" + mobilenum + "','" + phonenum + "','" + email + "','" + pad + "','" + work + "','" + ghousenum + "','" + gvillagename + "','" + gtahasil + "','" + gthana + "','" + gjila + "','" + gshahar + "','" + gstate + "','" + gdesh + "','" + gpincode + "','" + vhousenum + "','" + vvillagename + "','" + vtahasil + "','" + vthana + "','" + vjila + "','" + vshahar + "','" + vstate + "','" + vdesh + "','" + vpincode + "','" + signature + "','" + photo + "','" + remark + "','" + date + "')");
        }
        [WebMethod]
